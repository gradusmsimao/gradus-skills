...
**ECharts único** (motor primário) + **camada custom HTML/CSS/SVG** para o que não é chart. Substitui o Chart.js do exemplo (boxplot/heatmap são nativos no ECharts; no Chart.js exigem plugin) e ganha as formas canônicas do deck.
...
## 5. Texto editável
- **Todo texto** (lead title, subtítulos, células de tabela, callouts, OOI, rótulos, notas) é `contenteditable`; `input/blur` grava em `textOverrides`.
- **Valores derivados de gráfico/cálculo são READ-ONLY** (decisão do usuário): renderizar com `contenteditable=false` e estilo de "computado". Editar baseline/meta/cap se dá **pelos controles**, não digitando no número.
...
Bubble adicional criado pelo botão `+ Bubble` no header do deck. Mesma elipse do callout de oportunidade, **cor `--navy`** em vez de `--maroon`.
...
- **Implementação no deck**: `assets/deck-template.html` — campo `STATE.slides[].params` segue a estrutura de `makeBucketState`.
