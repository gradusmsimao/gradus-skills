...
## 0b. Capa de abertura (DECIDIDO 09/jun: capa entra · default = branca-PPTX)

Slide de abertura, **ligado por default quando o consultor informa cliente/pacote/data**. Não é bloqueante; o placar continua sendo o 1º slide de **conteúdo**. **Dois modos** — sempre só com tokens Gradus Nova (sem inventar cor; nada de gradiente):

### Modo A (DEFAULT) — capa branca fiel ao PPTX
É o modo padrão (calibrado contra o deck de PLA v3 e o `MAG001-Capa.pptx`): **fundo branco**, banda navy no topo, título navy centralizado, **logo do cliente centralizada**, subtítulo, data, disclaimer, logo Gradus inf-dir. Use sempre que houver logo/identidade do cliente.

```html
<section class="slide cover-w" data-id="capa" data-role="capa">
  <div class="cover-band"></div>                                  <!-- banda navy no topo -->
  <div class="cover-w-ttl">{TÍTULO DO DECK}</div>                  <!-- navy, centralizado, CAPS -->
  <img class="cover-w-logo" alt="{Cliente}" src="{logo do cliente, base64}">  <!-- centralizada -->
  <div class="cover-w-sub">{Pacote · Programa} — Validação de Diretrizes Orçamentárias</div>
  <div class="cover-w-date">{Cidade}, {data por extenso}</div>
  <div class="cover-w-disc">{disclaimer de uso/confidencialidade}</div>
...
```
```css
.slide.cover-w{ background:var(--paper); color:var(--navy); }       /* fundo BRANCO */
.slide.cover-w::before{ display:none; }                              /* sem a régua-de-título padrão */
.cover-band{ position:absolute; left:0; right:0; top:85px; height:5px; background:var(--navy); }
.cover-w-ttl{ position:absolute; left:64px; right:64px; top:18%; text-align:center;
  font-size:34pt; font-weight:700; color:var(--navy); line-height:1.15; text-transform:uppercase; }
.cover-w-logo{ position:absolute; left:50%; top:42%; transform:translateX(-50%);
  width:560px; height:auto; }                                        /* logo do cliente, centralizada */
.cover-w-sub{ position:absolute; left:64px; right:64px; top:74%; text-align:center;
  font-size:13pt; font-weight:700; color:var(--navy); }
.cover-w-date{ position:absolute; left:64px; right:64px; top:80%; text-align:center;
  font-size:11pt; color:var(--navy); }
.cover-w-disc{ position:absolute; left:64px; right:64px; bottom:48px; text-align:center;
  font-size:8.5pt; color:var(--grey-mid); line-height:1.4; }
```

### Modo B (alternativa) — capa navy IB
Fundo navy puro, título branco, barra orange. Usar quando não há logo do cliente ou se o consultor preferir o visual IB escuro.
```css
...
/* logo Gradus no footer em negativo sobre o navy */
```
**Regras (ambos os modos):** fonte **Gadugi** (sem exceção — ver §0); itálico inline em estrangeirismos do título; a capa **não** entra na reconciliação numérica nem no `printed_number` se o consultor preferir numerar a partir do placar (decidir com ele). **NUNCA gradiente** em nenhum dos modos. Default = Modo A (branca) quando há logo do cliente.
...
/* CONTEÚDO PADRONIZADO (v2): texto centralizado, 1 coluna, 3 linhas FIXAS:
   linha 1 "Oportunidade:" · linha 2 "R$ X,X MM/ano" · linha 3 "(YY%)".
   O % é compressão sobre o BASELINE DA CONTA; faixa → usar limite inferior.
   No v2 a elipse é flex-column (210×104), arrastável; ver output-and-interactivity-spec.md §13.5. */
...
/* caixa de premissa/limitação — alias âmbar do .infobox (ver §6b) */
...
## 6b. Componentes transversais opcionais — `.infobox` e `.kpi-strip` (DECIDIDO 09/jun, da v3)

> **Origem:** avaliação contra o deck de PLA v3 (`melhoria skill/AVALIACAO_SKILL_VS_DASH_PLA_v3.md`, D1.3/D1.4). Dois componentes que a v3 usa, úteis em `análise`/`vgc`, **oficializados em versão Gradus-sóbria** (só tokens, fonte Gadugi). **Distinção crítica:** o que está banido por `output-spec §12.1` é o **"KPI card decorado do dashboard MAG001"** (com ícones, sombras, charts embutidos, tipografia própria) — o `.kpi-strip` abaixo é uma **faixa de âncoras numéricas sóbria** (borda-topo + valor + label, tokens Gradus), permitida. Ambos são **opcionais** (não-canônicos obrigatórios); use quando dão régua à mensagem.

**`.infobox` — caixa de leitura/insight/premissa (3 variantes).** Generaliza o `.premise` (§6): mesma família, 3 cores de borda-esquerda. Usar para nota de leitura, insight, contexto ou limitação de premissa — **não** confundir com o `.callout-opp` bordô (que é o valor da oportunidade).
```css
.infobox{ padding:10px 15px; border-left:4px solid var(--orange); background:#FFF8E1;
  color:var(--navy); font-size:var(--fs-body); line-height:1.45; border-radius:0 4px 4px 0; }
.infobox.grey{ background:var(--zebra);  border-left-color:var(--navy); }   /* nota neutra */
.infobox.blue{ background:#EAF0F8;       border-left-color:var(--blue); }   /* contexto */
.infobox b{ color:var(--maroon); }                                          /* ênfase bordô parcial */
/* .premise (§6) = alias âmbar deste componente (limitação de premissa). */
```

**`.kpi-strip` — faixa de 2-4 âncoras numéricas no topo da análise.** Borda-topo navy (variantes orange/maroon/blue por semântica), valor navy bold, label cinza. Sóbrio, sem ícone/sombra/chart.
```html
<div class="kpi-strip">
  <div class="kpi"><div class="v">R$ 48,1 MM</div><div class="l">investimento sem retorno</div></div>
  <div class="kpi a"><div class="v">R$ 44 MM</div><div class="l">desperdício 3P</div></div>
  <div class="kpi t"><div class="v">16,9%</div><div class="l">economia do PLA</div></div>
</div>
```
```css
.kpi-strip{ display:flex; gap:14px; }
.kpi{ flex:1; background:var(--zebra); border-top:3px solid var(--navy); padding:9px 12px; }
.kpi .v{ font-size:19pt; font-weight:700; color:var(--navy); line-height:1.1; }
.kpi .l{ font-size:9pt; color:var(--grey-mid); line-height:1.2; margin-top:3px; }
.kpi.a{ border-top-color:var(--orange); }   /* atenção/destaque */
.kpi.c{ border-top-color:var(--maroon); }   /* crítico/negativo */
.kpi.t{ border-top-color:var(--blue); }     /* positivo/total */
```
**Regras:** valores no `.kpi` que vierem de cálculo são `computed`/read-only (output-spec §5); fonte **Gadugi** sempre; o `.kpi-strip` não substitui o quadro dominante do slide — é âncora numérica complementar.
...
```
**Colunas:** as **5 acima são o núcleo obrigatório** (Oportunidade primeiro). Colunas **extras opcionais** (`#`, `Impacto`, `Status`) são aceitas desde que as 5-núcleo estejam presentes e a `Oportunidade (R$)` reconcilie com o mapa/placar — ver `canonical-templates.md §6`.
