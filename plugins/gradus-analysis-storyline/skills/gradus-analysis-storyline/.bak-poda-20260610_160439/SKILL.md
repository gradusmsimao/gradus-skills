...
| `references/output-and-interactivity-spec.md` | Spec de saída deck-mode HTML, engine ECharts, JSON backbone, edição inline, **análise viva + mapa-mestre como camadas opt-in (§10)**, **métricas validadas do template (§11)**, **referência de interatividade MAG001 (§12)**, **arquitetura do template v2: barra única no topo, toggle Apresentar⇄Editar, `data-layers`, persistência leve, delete de bubble (§13)** e **pipeline de assembler + validação headless (§14)**. Leitura obrigatória antes de gerar HTML. |
...
- `assets/deck-template.html` — **template v2** (validado em campo): chrome de produção (capa branca montada, header com crumb + **barra única no topo, toggle Apresentar⇄Editar**, overlay de índice de slides, nav que pula `data-backup`, `renderWaterfall` robusto) + canônicos (callout bordô + drag, badges) + **camadas opt-in via `data-layers` no `<body>`** (`edit persist callout badges` default; `master-map`/`live` ligáveis). Offline (Chart.js embutido). **Clonar e injetar conteúdo — não criar HTML do zero.** Detalhes: `output-and-interactivity-spec.md §13`. Backup do anterior: `assets/deck-template.html.pre-v2.bak`.
