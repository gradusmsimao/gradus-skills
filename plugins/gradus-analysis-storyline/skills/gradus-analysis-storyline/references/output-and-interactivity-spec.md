...
### 11.4 Callout de oportunidade (bubble bordô) — geometria + contrato de drag

**Geometria/comportamento e formato do conteúdo (3 linhas fixas) → `§13.5` + `design-tokens-per-template.md`.** Aqui fica só o invariante anti-bug:
...
> 1. **`width`/`height` SEMPRE explícitos** no `.callout-opp` (a forma não depende do conteúdo).
> 2. O handler de drag seta **apenas `left`/`top`** e **limpa `right`/`bottom`** (`right='auto'; bottom='auto'`); **nunca** toca `width`/`height`.
> 3. Nenhum elemento nasce com `left` **e** `right` sem `width` fixo (dois referenciais → estica). Vale p/ callout, bubble navy e qualquer caixa draggável.
> Checado no QA (`qa-checklist.md §4.6/§7`) + smoke-test (`§7.2`).

### 11.5 Bubble custom navy ("+ Bubble") → ver §13.5

Mecânica completa (criação, texto em `.bubble-text` interno, `×` fora do editable, delete em 3 vias, drag que pula texto/×) está em **§13.5**. Geometria/cor: mesma elipse do callout, `--navy` em vez de `--maroon`.
...
### 11.9 Header fixo do deck → ver §13.3

Barra **única no topo** com toggle Apresentar⇄Editar; botões e visibilidade por modo/camada estão em **§13.3** (`btn-mode`, `btn-index`, `btn-prev/next`, `btn-bubble`, `btn-download`, `btn-reset`, `btn-master`/`btn-export-json` condicionais). Bindings no boot `initV2()`.

### 11.10 Atalhos de teclado

- `→` / `PageDown`: próximo slide · `←` / `PageUp`: slide anterior.
- `Delete` / `Backspace`: remove bubble custom selecionado **sem** `contenteditable` ativo (detalhe e guard em §13.5).
- Atalhos não disparam enquanto se digita em `contenteditable`.
...
| Bubble custom navy (botão "+ Bubble") | §13.5 |
...
- Camadas: `edit` (edição inline), `persist` (localStorage), `callout`/`badges` (sempre on na prática), `export` (export de dados por elemento em .xlsx — §13.8, default on), `master-map` (mapa-mestre/classificação no placar), `live` (análise viva + Exportar JSON).
...
### 13.7 Slides de BACKUP — atributo `data-backup` (mecanismo de navegação)

É como o **conceito canônico de BACKUP** (badge/função — `narrative-flow.md §4`, `analise-patterns.md §4b`) se liga ao **comportamento do deck**: o slide de lastro existe no deck (acessível), mas **não interrompe a apresentação principal**.

- **Marcação:** `<section class="slide" data-id="…" data-backup>` no slide de lastro/detalhe.
- **Navegação:** `nextMain()`/`prevMain()` (setas ← →, botões Anterior/Próximo) **pulam** os slides com `data-backup` (`isBackup(k)` testa o atributo). Eles ficam acessíveis só pelo **índice (☰ Slides)** ou navegação direta — exatamente o papel do backup ("o sócio decide exibir").
- **Relação com o badge `BACKUP` e com `hidden`:** os três são independentes e combináveis — `data-backup` é o **mecanismo de navegação** (pula na sequência); o **badge `BACKUP`** é a **etiqueta visual** no canto; `hidden` (na taxonomia da biblioteca) é a decisão de exibição. Um slide de lastro tipicamente leva `data-backup` + badge `BACKUP`, mas pode ter um sem o outro.
- **Impacto no `printed_number`/contador:** o slide conta como página do deck (aparece no índice e no total); só não é alcançado pela navegação sequencial.

### 13.8 Export de dados por elemento — `.xlsx` (camada `export`, SheetJS embutido)

Atende ao §6.1 ("botão exportar por elemento"). **SheetJS (`xlsx.mini.min.js`, ~250KB) embutido offline** no template (sem CDN). Concretiza o que a §6/§12.4 prometiam.

- **Gatilho por elemento:** todo bloco marcado `class="exportable"` ganha um mini-botão **`⬇ dados`** (`.el-export`) no canto. Click → `.xlsx` com aba **Dados** + aba **Config** (rastreabilidade: deck, slide, tipo, data/hora — espelha `buildConfigSheet` §12.2).
- **Visibilidade:** o botão **só aparece no modo Editar E com a camada `export` ligada**, no **hover** do elemento (`body[data-mode="edit"][data-layers~="export"] .exportable:hover > .el-export`). No **modo Apresentar some** (decisão do consultor: é ferramenta de trabalho, não de apresentação).
- **De onde vêm os dados (2 fontes):**
  1. **Tabela** — se o `.exportable` contém/é uma `<table>`, `tableToAoA()` lê as linhas do DOM (converte célula numérica `1.234,5`→número; texto fica texto). Cobre placar/mapa/OOI/amostra.
  2. **Gráfico** — como os charts são **SVG nativo** (dados não num objeto JS), o elemento declara os dados num atributo **`data-export-series='{"cols":[…],"rows":[[…]]}'`** (JSON). O injetor/assembler preenche isso com a mesma série que alimentou o SVG. Sem o atributo, um gráfico não é exportável (alerta).
- **Camada `export`** entra no `data-layers` default (`edit persist callout badges export`). Para um deck sem export (mais leve, sem os 250KB), remover `export` do `data-layers` **e** não embutir o SheetJS.
- **Convenção pro gerador:** marcar `.exportable` em torno de cada tabela/quadro que valha exportar; em gráfico, sempre emitir `data-export-series` espelhando os dados renderizados (mantém export = o que está na tela).
