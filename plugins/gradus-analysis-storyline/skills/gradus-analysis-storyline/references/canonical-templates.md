...
**Anatomia:** o template **mais sóbrio** — tabela navy puro, **sem gráficos/callouts/shade**. **5 colunas-núcleo (obrigatórias):**
...
- **Colunas extras OPCIONAIS (DECIDIDO 09/jun, da v3):** além das 5-núcleo, são aceitas colunas adicionais como **`#`** (ordem), **`Impacto`** (qualitativo) e **`Status`** — desde que as 5-núcleo estejam presentes. A `Oportunidade (R$)` deve **reconciliar com o mapa-de-análises/placar**. Anti-padrão (visto no rascunho da v3): trocar `Oportunidade (R$)` e `Origem` por `Descrição/Impacto` — isso **perde** a âncora numérica e a justificativa; manter as 5-núcleo e só acrescentar extras.
