...
## 7. Checagens de BUILD do template v2 (pipeline por assembler — `output-and-interactivity-spec.md §13/§14`)
...
**7.1 Integridade do HTML gerado (BLOQ):**
...
**7.2 Smoke-test de INTERAÇÃO (BLOQ — Chrome headless, `assets/pipeline/smoke_v2.js` + `smoke_del.js` + `smoke_export.js`):**
...
- 🔴 **export de dados (§13.8):** `XLSX` carregado (offline); `.el-export` escondido no modo Apresentar e revelado no hover no modo Editar (camada `export`); `tableToAoA()` extrai a tabela → `XLSX.write` sem erro; `data-export-series` parseia no gráfico.

**7.3 LER os screenshots (não confiar em "0 erros"):** `validate.js` salva 1 PNG por slide; abrir os de slides com callout/tabela/SVG e conferir sobreposição, texto cortado e o callout bordô no formato fixo de 3 linhas.
