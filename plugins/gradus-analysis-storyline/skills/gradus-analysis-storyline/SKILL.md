...
**Modo B interativo (sub-modo de 1ª classe — DECIDIDO 09/jun, da v3):** o Modo B **pode** ter interatividade leve de **exibição** sem virar análise viva — toggles de série/métrica/layout no gráfico (ex.: "Investimento / Nº de SKUs", "1P / 3P / Ambos", "Lado a lado / Empilhado"). Isso **troca o que o gráfico mostra** sobre dados que já vêm prontos (`DECK`/`DECK.Vx`), **sem** recalcular oportunidade. É o que a v3 faz e é legítimo — não é workaround. A oportunidade (callout/mapa/placar) permanece fixa; só a visualização alterna.
...
> 📉 **Carregamento PROGRESSIVO (orçamento de contexto).** NÃO leia tudo no início. Leia **só o que a peça atual exige**, na hora. As referências são grandes (juntas ~1.900 linhas) — carregar todas antes da Fase 2 estoura o contexto sem necessidade. A coluna **"Quando carregar"** manda. Regra de ouro: antes de gerar a v1, leia apenas `canonical-templates.md`; o resto entra quando você for gerar a peça/HTML correspondente. Em cada referência longa, **pule para a §/seção citada** em vez de ler o arquivo inteiro.

| Arquivo | Quando carregar | O que tem |
|---|---|---|
| `references/canonical-templates.md` | **Início (única obrigatória upfront)** | Estrutura dos 6 templates canônicos (placar / buildup / VGC / mapa / análise / OOI) |
| `references/narrative-flow.md` | Ao montar a **sequência** da conta | Inferência setup/oportunidade/backup, transições entre análises/contas, invariante de Total |
| `references/analise-patterns.md` | Ao gerar **slide de análise** | Taxonomia forma×função (90 clusters), seleção de forma, fraseado de lead title, badges (5) |
| `references/visao-geral-patterns.md` | Ao gerar **VGC** | 3 motores (árvore foto-texto / waterfall-escada / barras benchmark), fraseado por pacote |
| `references/ooi-patterns.md` | Ao gerar **OOI** | Anatomia da lista (5 colunas, ordem real, verbos infinitivo), fraseado de iniciativas |
| `references/design-tokens-per-template.md` | Ao **gerar/estilizar HTML** (pular pra seção do template em uso) | Tokens exatos (cor/tipografia/geometria), snippets por template, formato fixo do callout |
| `references/output-and-interactivity-spec.md` | Ao **gerar HTML** — ler **só as §§ relevantes**, não o arquivo todo | §1 saída/no-scroll · §2 engine · §3 JSON · §10 camadas an.viva/mapa-mestre (opt-in) · §11 métricas do template · §12 MAG001 (só mecânica) · **§13 arquitetura v2** (barra única, toggle, `data-layers`, persistência, bubble/delete) · **§14 pipeline assembler+validação** |
| `references/MAG001-Dashboard-Comparacao-Filiais-v12.html` | Só se for ligar a camada **`live`** (análise viva) | Mecânica de interatividade (cluster/meta/cap/export). ⚠️ NUNCA estilização — visual vem do design-tokens + §11. Ver §12. |
| `references/pyramid-principle.md` | No **QA** (e se precisar destilar lead title) | 8 invariantes de raciocínio, SCQR, dedução vs indução |
| `references/qa-checklist.md` | Antes da **entrega** (e §7 a cada build do assembler) | Checklist 4 dimensões + §5 smoke-test + §7 build-checks |
| `assets/pipeline/README.md` | Ao montar via **assembler** | Como o template v2 é gerado + princípios do pipeline |

### Assets internos (própria skill)

A skill já tem **tudo necessário** na pasta `assets/`:

- `assets/deck-template.html` — **template v2** (validado em campo): chrome de produção (capa branca montada, header com crumb + **barra única no topo, toggle Apresentar⇄Editar**, overlay de índice de slides, nav que pula `data-backup`, `renderWaterfall` robusto) + canônicos (callout bordô + drag, badges) + **export de dados por elemento em .xlsx** (SheetJS embutido, botão `⬇ dados` só no modo Editar — §13.8) + **camadas opt-in via `data-layers` no `<body>`** (`edit persist callout badges export` default; `master-map`/`live` ligáveis). Offline (Chart.js + SheetJS embutidos). **Clonar e injetar conteúdo — não criar HTML do zero.** Detalhes: `output-and-interactivity-spec.md §13`. Backup do anterior: `assets/deck-template.html.pre-v2.bak`.
- `assets/pipeline/` — **pipeline de montagem por assembler** (Python) + validadores headless: `build_template_v2.py` (gera o template), `build_deck_EXEMPLO.py` (exemplo real de deck de conta), `validate.js`/`smoke_v2.js`/`smoke_del.js`. Método: "não editar HTML à mão — montar por assembler e validar headless". Ver `output-and-interactivity-spec.md §14`.
...
Saída sempre em **HTML único deck-mode 16:9** (1280×720 com `transform: scale()` para letterbox). Esqueleto base em `assets/deck-template.html` (template v2) — clonar e injetar conteúdo. Não criar HTML do zero. A montagem se dá por **assembler** (`assets/pipeline/`), não edição manual; a interatividade segue a **arquitetura de camadas** (`output-and-interactivity-spec.md §13`): barra única no topo, toggle Apresentar⇄Editar, edição só de texto narrativo, `data-layers` no `<body>`.
